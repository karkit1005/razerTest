 ...
 class ADBObjectStore {
   ...
   public async createDatabase(database_schema: any, records: any) {
     ...
     request.onupgradeneeded = (event) => {
       ...
       indexes.forEach((index : any) => objectStore.createIndex(index.name, index.keyPath, { multiEntry: index.multiEntry, unique: index.unique }));
 
       objectStore.transaction.oncomplete = (event) => {
         let recordObjectStore = db.transaction(name, "readwrite").objectStore(name);
